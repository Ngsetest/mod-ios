...
class HalfModalInteractiveTransition: UIPercentDrivenInteractiveTransition {
    ...
    init(viewController: UIViewController, withView view: UIView, presentingViewController: UIViewController?) {
        ...
        self.panGestureRecognizer.addTarget(self, action: #selector(self.onPan))
    ...
    @objc func onPan(pan: UIPanGestureRecognizer) {
        ...
        switch pan.state {
        case .began:
            let navCon = presentingViewController as! UINavigationController
            if type(of: navCon.visibleViewController!) != ProcessOrderViewController.self {
                cancel()
                return
            }
        ...
        case .ended, .cancelled:
            if pan.state == .cancelled || !self.shouldComplete { cancel() }
            else { finish() }
            break
